...
#+end_src
** Reddit
[[https://www.reddit.com/][Reddit]] search engine. Its NSFW search filter only works on alternative front ends.
...
** Lemmy
Search engine for [[https://join-lemmy.org/][Lemmy]], a link aggregator for the Fediverse. Do note this search engine has a rather esoteric query parameter structure and needs for all of its parameters to be supplied in the search URL, otherwise it throws a server error.
